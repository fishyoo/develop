...
变量名		【是否定义，与定义的是否一致，如: a和arr。】【变量名是否符合命名规范】
...
【code review】
制表符		【tab键替换为空格】
默认值		【配置参数要赋值默认值；ERROR打印重要参数；读配置为空或异常时return -1；】
作用域		【class的成员是否要public】
c++			【使用stringstream构造字符串】【简单构造函数建议放头文件】
分拆函数
初始化		【变量要初始化】
枚举、const	【特殊数字含义用枚举、常量】
bool		【二值变量】
注释		【这里的authorUid用错类型了，加个FIXME备注下吧】
